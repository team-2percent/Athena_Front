...
//     // 펀딩 시작일 선택 (DatePicker)
//     cy.get('[data-cy="datepicker-start"]').click();
//     cy.get('button').contains('12').click(); // 6월 5일 선택 (예시)

//     // 펀딩 종료일 선택 (DatePicker)
//     cy.get('[data-cy="datepicker-end"]').click();
//     cy.get('button').contains('19').click(); // 6월 10일 선택 (예시)

//     // 배송 예정일 선택 (DatePicker)
//     cy.get('[data-cy="datepicker-delivery"]').click();
//     cy.get('button').contains('26').click(); // 6월 17일 선택 (예시)
...
//     cy.get('input#targetAmount').clear().type('1000000');
//     // cy.get('[data-cy="datepicker-start"]').click();
//     // cy.get('button').contains('12').click(); // 펀딩 시작일 미입력
//     cy.get('[data-cy="datepicker-end"]').click();
//     cy.get('button').contains('19').click();
//     cy.get('[data-cy="datepicker-delivery"]').click();
//     cy.get('button').contains('26').click();
...
describe('프로젝트 입력란 유효성 검사', () => {
  ...
  describe('상품 요약', () => {
    it('10자 미만 입력 시 에러 메시지 노출', () => {
      ...
      cy.contains('10자 이상 입력해주세요').should('be.visible');
    });
    it('50자 초과 입력 시 에러 메시지 노출', () => {
      cy.get('textarea#description').clear().type('A'.repeat(51));
      cy.contains('50자 이내로 입력해주세요').should('be.visible');
    });
    it('정상 입력 시 에러 메시지 미노출', () => {
      cy.get('textarea#description').clear().type('이것은 정상적인 상품 요약입니다.');
      cy.contains('10자 이상 입력해주세요').should('not.exist');
      cy.contains('50자 이내로 입력해주세요').should('not.exist');
    });
  });
